 ...
 console.log(place);
    return (
        <Card elevation={6}>
            <CardMedia 
                style={{height: 350}}
                image={place.photo ? place.photo.images.large.url : 'http://www.foodserviceandhospitality.com/wp-content/uploads/2016/09/Restaurant-Placeholder-001.jpg'}
                title={place.name}
            />
            <CardContent>
                ...
                {place?.cuisine?.map(({name}) =>(
                    <Chip key={name} size="small" label={name} className={classes.chp}/>
                ))}
